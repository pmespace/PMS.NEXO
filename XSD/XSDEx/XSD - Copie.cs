...
namespace XSDEx
{
	...
	public class XSD
	{
		...
		class MyCodeTypeDeclarations : Dictionary<string, CodeTypeDeclaration> { }
		...
		public bool AnalyseXSD(XSDSettings settings, bool mixFiles)
		{
			...
			try
			{
				if (mixFiles)
				{
					...
					return ProcessFile(settings, xsds, fileName, tags, codeTypeDeclarations, true);
				}
				else
				{
					bool fOK = true;
					...
					for (int i = 0; i < settings.Files.Count; i++)
					{
						...
						xsds.Add(xsd);
						xsds.Compile(null, true);
						fOK = fOK && ProcessFile(settings, xsds, fileName, tags, codeTypeDeclarations, i == settings.Files.Count - 1);
					}
					return fOK;
				}
			}
		...
		private bool ProcessFile(XSDSettings settings, XmlSchemas xsds, string fileName, MyTags tags, MyCodeTypeDeclarations codeTypeDeclarations, bool saveTags)
		{
			try
			{
				// create the context
				...
				PostProcess(settings, CodeNamespace, tags, codeTypeDeclarations, saveTags);
		...
		private static void PostProcess(XSDSettings settings, CodeNamespace CodeNamespace, MyTags tags, MyCodeTypeDeclarations codeTypeDeclarations, bool addTags)
		{
			...
			try
			{
				...
				foreach (CodeTypeDeclaration codeType in CodeNamespace.Types)
				{
					...
					if (add && addInterface && settings.DeclareClassInterface)
					{
						...
						try
						{
							codeTypeDeclarations.Add(itf.Name, itf);
						}
						catch (Exception)
						{
						}
...
#if USEDICT
					codeTypeDeclarations.Add(ctd.Name, ctd);
#else
				...
				{
#if USEDICT
					List<CodeTypeDeclaration> ctds = new List<CodeTypeDeclaration>();
					foreach (KeyValuePair<string, CodeTypeDeclaration> k in codeTypeDeclarations)
						ctds.Add(k.Value);
					CodeNamespace.Types.AddRange(ctds.ToArray());
#else
