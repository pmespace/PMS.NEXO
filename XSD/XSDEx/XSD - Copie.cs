...
namespace XSDEx
{
	...
	public class XSD
	{
		...
		public bool AnalyseXSD(XSDSettings settings, bool mixFiles, Label lbl)
		{
			...
			try
			{
				...
				if (mixFiles)
				{
					...
					return ProcessFile(settings, xsds, fileName, true, msg);
				}
				else
				{
					...
					for (int i = 0; i < settings.Files.Count; i++)
					{
						...
						fOK = fOK && ProcessFile(settings, xsds, fileName, i == settings.Files.Count - 1, msg);
					}
					return fOK;
				}
			}
		...
		/// <returns></returns>
		private bool ProcessFile(XSDSettings settings, XmlSchemas xsds, string fileName, bool lastFile, Message msg)
		{
			try
			{
				msg.Header = fileName;
