...
namespace XSDEx
{
	static public class XSDExtensions
	{
		static string TAB(int _i_) { string s = string.Empty; for (int i = 0; i < _i_; i++) s += Chars.TAB; return s; }
		static string Add(string _n_, string _s_ = null, int _i_ = 0) => TAB(_i_) + $"{(_n_.IsNullOrEmpty() ? string.Empty : $"{_n_}")}{(_s_.IsNullOrEmpty() ? string.Empty : $": {_s_}")}" + Chars.CRLF;
		static string AddB(string _n_, bool _b_, int _i_ = 0) => (_b_ ? Add(_n_, null, _i_) : string.Empty);

		public static string AsString(this CodeParameterDeclarationExpressionCollection cpdec)
		{
			string str = string.Empty;
			foreach (CodeParameterDeclarationExpression k in cpdec)
				str += $"{k.Name}: {k.Type}";
			return str;
		}
		public static string AsString(this CodeTypeDeclaration ctd)
		{
			string str = string.Empty, strP = string.Empty, strF = string.Empty, strM = string.Empty, strS = string.Empty, strE = string.Empty, strO = string.Empty;

			str += Add(ctd.Name, nameof(ctd.Name));
			str += AddB(nameof(ctd.IsClass), ctd.IsClass, 1);
			str += AddB(nameof(ctd.IsEnum), ctd.IsEnum, 1);
			str += AddB(nameof(ctd.IsInterface), ctd.IsInterface, 1);
			str += AddB(nameof(ctd.IsStruct), ctd.IsStruct, 1);
			str += AddB(nameof(ctd.IsPartial), ctd.IsPartial, 1);

			foreach (CodeTypeMember k in ctd.Members)
			{
				if (k is CodeMemberProperty)
				{
					var c = k as CodeMemberProperty;
					strP += Add(nameof(k.Name), k.Name, 2);
					strP += Add(nameof(c.Type), c.Type.BaseType, 3);
					strP += AddB("array", 0 < c.Type.ArrayRank, 3);
					strP += AddB("data", 0 == c.Type.ArrayRank, 3);
				}
				else if (k is CodeMemberField)
				{
					var c = k as CodeMemberField;
					strF += Add(nameof(k.Name), k.Name, 2);
					strF += Add(nameof(c.Type), c.Type.BaseType, 3);
					strF += AddB("array", 0 < c.Type.ArrayRank, 3);
					strF += AddB("data", 0 == c.Type.ArrayRank, 3);
				}
				else if (k is CodeMemberMethod)
				{
					var c = k as CodeMemberMethod;
					strM += Add(nameof(k.Name), k.Name, 2);
					strM += Add(nameof(c.Parameters), c.Parameters.AsString(), 3);
					strM += Add(nameof(c.ReturnType), c.ReturnType.ToString(), 3);
				}
				else if (k is CodeSnippetTypeMember)
				{
					var c = k as CodeSnippetTypeMember;
					strS += Add(nameof(k.Name), k.Name, 2);
					strS += Add(nameof(c.Text), c.Text, 3);
				}
				else if (k is CodeMemberEvent)
				{
					var c = k as CodeMemberEvent;
					strE += Add(nameof(k.Name), k.Name, 2);
				}
				else
				{
					strO += Add(k.Name, nameof(k.Name), 2);
				}
			}
			return str += Add("Properties", null, 1) + Add(strP, null)
				+ Add("Fields", null, 1) + Add(strF)
				+ Add("Methods", null, 1) + Add(strM)
				+ Add("Events", null, 1) + Add(strE)
				+ Add("Snippet", null, 1) + Add(strS)
				+ Add("Others", null, 1) + Add(strO);
		}
	}
	...
	public class XSD
	{
		...
		private const string XML_ELEMENT_ATTRIBUTE = "System.Xml.Serialization.XmlElementAttribute";
		private const string XML_ATTRIBUTE_ATTRIBUTE = "System.Xml.Serialization.XmlAttributeAttribute";
		private const string XML_ARRAY_ATTRIBUTE = "System.Xml.Serialization.XmlArrayAttribute";
		private const string JSON_IGNORE_ATTRIBUTE = "Newtonsoft.Json.JsonIgnore";
		private const string JSON_CONVERTER_ATTRIBUTE = "Newtonsoft.Json.JsonConverter";
		private readonly string[] ATTRIBUTE_VALUE_SEPARATORS = new string[] { " ", ";" };
		#endregion
		...
		public bool AnalyseXSD(AnalyseSettings settings)
		{
			...
			try
			{
				...
				else
				{
					...
					settings.xsdSettings.Nmspace += (XSDSettings.NameSpace.None == settings.xsdSettings.NamespaceToDeclare ? null : $".{settings.xsdSettings.NamespaceToDeclare}") + (settings.xsdSettings.UseVersion && !settings.xsdSettings.Version.IsNullOrEmpty() ? $".v{settings.xsdSettings.Version}" : string.Empty);
...
#if DEBUG
				using (StreamWriter sw = new StreamWriter("Types.txt", false))
					foreach (CodeTypeDeclaration k in codeNamespace.Types)
					{
						sw.WriteLine(k.AsString());
					}
#endif
		...
		/// <param name="lastFile"></param>
		/// <param name="msg"></param>property.Name + "Specified"
		/// <returns></returns>
		private CodeNamespace PostProcess(XSDSettings settings, XSDParams parameters, CodeNamespace codeNamespace, bool lastFile, Message msg)
		{
			...
			try
			{
				string lengthMethod = NexoXSDStrings.AccessorCount,
				...
				// enum to always ignore
				List<string> AlwaysIgnoreThoseENums = new List<string>() { NexoXSDStrings.EnumBegin, NexoXSDStrings.EnumEnd };

				// indicates whether the Document class exists or not
				...
				foreach (CodeTypeDeclaration codeType in codeNamespace.Types)
				{
					...
					try
					{
						...
						if (visible && (codeType.IsClass || codeType.IsStruct))
						{
							...
							// determines whether the object of that class holds its own value or not
							bool hasXmlText = false;
							//CodeAttributeDeclaration cadt = new CodeAttributeDeclaration("System.Xml.Serialization.XmlTextAttribute");
							//foreach (CodeTypeMember member in codeType.Members)
							//{
							//	if (member is CodeMemberProperty)
							//		foreach (CodeAttributeDeclaration decl in member.CustomAttributes)
							//			hasXmlText = hasXmlText || decl.Name.Contains("XmlTextAttribute");
							//}
							...
							foreach (CodeTypeMember member in codeType.Members)
							{
								...
								if (member is CodeMemberProperty)
								{
									...
									CodeMemberProperty propertyEx = null;
									string propertyExName = string.Empty;

									// first add the property name to the list of tags
									try
									{
										if (!tags.ContainsKey(member.Name))
									...
									CodeTypeDeclaration ctdOriginallyDeclaredInNamespace = null;
									try
									{
										ctdOriginallyDeclaredInNamespace = typesDeclaredInsideNamespace[property.Type.BaseType];
										//if (null != ctdOriginallyDeclaredInNamespace && ctdOriginallyDeclaredInNamespace.IsEnum)
										//	ctdOriginallyDeclaredInNamespace = null;
									}
									catch (Exception ex)
									{
									}
									...
									#region property to field management
									...
									property.Type = propertyField.Type = t;
									#endregion

									#region create enum special properties if needed
									Func<bool> IsEnumButNotAsString = () => null != ctdOriginallyDeclaredInNamespace && ctdOriginallyDeclaredInNamespace.IsEnum && !settings.EnumAsString;

									Func<CodeMemberProperty, CodeStatement, List<CodeStatement>> SetEnumValueStatementsAsList = (CodeMemberProperty _n_, CodeStatement _cs_) =>
									{
										List<CodeStatement> thenCS = new List<CodeStatement>()
										{
											new CodeAssignStatement(
												new CodePropertyReferenceExpression(
													new CodeThisReferenceExpression(),
													property.Name),
												settings.EnumAsString
										...
										if (null != _cs_) thenCS.Add(_cs_);

										List<CodeStatement> _ll_ = new List<CodeStatement>()
										{
											new CodeCommentStatement($"{bXSD}"),
											// declare List
											new CodeVariableDeclarationStatement(
												new CodeTypeReference("System.Collections.Generic.List",
												...
												new CodeObjectCreateExpression(
													...
													new CodeMethodInvokeExpression(
														...
														"GetNames",
														new CodeTypeOfExpression(_n_.Type)))),
											new CodeConditionStatement(
												new CodeBinaryOperatorExpression(
													new CodeMethodInvokeExpression(
														new CodeVariableReferenceExpression(listVariableName),
														"Contains",
														new CodeMethodInvokeExpression(
															new CodePropertySetValueReferenceExpression(),
															"ToString")),
													// && value != _begin && value != _end
															...
															new CodeMethodInvokeExpression(
																...
																new CodeExpression[]
																{
																	new CodeMethodInvokeExpression(
																		new CodePropertySetValueReferenceExpression(),
																		"ToString"),
																	new CodePrimitiveExpression(NexoXSDStrings.EnumBegin),
															...
															new CodeMethodInvokeExpression(
																...
																new CodeExpression[]
																{
																	new CodeMethodInvokeExpression(
																		new CodePropertySetValueReferenceExpression(),
																		"ToString"),
																	new CodePrimitiveExpression(NexoXSDStrings.EnumEnd),
																	new CodePrimitiveExpression(true)
																}),
															CodeBinaryOperatorType.IdentityInequality,
															new CodePrimitiveExpression(0)))),
												// then
												thenCS.ToArray<CodeStatement>().ToArray()),
										};

										_ll_.Add(new CodeCommentStatement($"{eXSD}"));
										return _ll_;
									};

									Func<CodeMemberProperty, CodeStatement, CodeStatement[]> SetEnumValueStatements = (CodeMemberProperty _n_, CodeStatement _cs_) => SetEnumValueStatementsAsList(_n_, _cs_).ToArray<CodeStatement>().ToArray();
									...
									if (null != ctdOriginallyDeclaredInNamespace && ctdOriginallyDeclaredInNamespace.IsEnum)
									{
										...
										{
											// create a specific property to manipulate this property as a real enum
											propertyExName = "AsEnum";
											propertyEx = CreatePropertyMember(property.Name + propertyExName, property.Type, MemberAttributes.Public | MemberAttributes.Final);
											addedMembers.Add(propertyEx);
											// that property can only set the enum property and is not subject to be retrieved
											propertyEx.SetStatements.Clear();
											propertyEx.SetStatements.AddRange(SetEnumValueStatements(propertyEx, null));
											// set the property type to string 
											property.Type = propertyField.Type = new CodeTypeReference(typeof(string));
											// enum is translated to string, as ctdOriginallyDeclaredInNamespace is used to know whether "HasBeenSet" is used we must set ctdOriginallyDeclaredInNamespace to null to avoid setting that property
											ctdOriginallyDeclaredInNamespace = null;
										}
										else
										{
											// add default serialization processing
											AddAttribute(property.CustomAttributes,
												new CodeAttributeDeclaration(
													new CodeTypeReference(typeof(System.ComponentModel.DefaultValueAttribute)),
													new CodeAttributeArgument(
														new CodePrimitiveExpression(0))
													));
											// indicate default value is not serialized
											AddAttribute(property.CustomAttributes,
												new CodeAttributeDeclaration(
													new CodeTypeReference(typeof(Newtonsoft.Json.JsonPropertyAttribute)),
													new CodeAttributeArgument(
														"DefaultValueHandling",
														new CodeCastExpression(
															typeof(Newtonsoft.Json.DefaultValueHandling),
															new CodePrimitiveExpression((int)Newtonsoft.Json.DefaultValueHandling.Ignore))
													)));
										}
									}
									#endregion

									#region property array type conversion if requested
									// if an array and conversion to List is requested let's change its type
									if (IsArray(property.Type) && settings.UseListInsteadOfArray && !IsByteType(property.Type))
									{
										ctr = new CodeTypeReference(typeof(List<>));
										ctr.TypeArguments.Add(property.Type.BaseType);
										property.UserData.Add(isArrayElementType, property.Type.BaseType);
										property.Type = propertyField.Type = ctr;
										property.UserData.Add(isList, true);

										/*
										 * never happens
										 */
										if (IsByteType(property.Type))
											property.UserData.Add(isByteSequence, true);
									}
									//else if (IsArray(property.Type) && settings.UseListInsteadOfArray && IsByteType(property.Type))
									//{
									//	ctr = new CodeTypeReference(typeof(string));
									//	property.Type = propertyField.Type = ctr;
									//	property.UserData.Add(isByteSequence, true);
									//}
									#endregion

									#region search default value
									...
									if (null != (caa = GetAttributeArgument(property, cad, caa))
										&& settings.ConvertStringIntegralToIntegral
										&& IsStringType(property.Type)
										&& !IsArray(property.Type)
										&& !property.UserData.Contains(isList)
										)
									{
									...
									#region XML attribute to element management
									try
									{
										/*
										 * for every attribute we create the same as en element so that if specified as an element in a message
										 * its value will be catched anyway but feeding the attribute data
										 * the element will not de serialised (in xml or json) but the attribute will
										 */
										cad = new CodeAttributeDeclaration(new CodeTypeReference(typeof(System.Xml.Serialization.XmlAttributeAttribute)));
										if (
											null != (cad = LookForAttribute(property, cad))
											&& settings.AttributeAsElement
											//&& !IsArray(property.Type)
											&& (IsIntegralType(property.Type) || IsStringType(property.Type))
											//&& property.Type != new CodeTypeReference(typeof(object[]))
											)
										{
											/*
											 * determine the type of the attribute and the element
											 */
											//CodeTypeReference typeAttribute, typeElement;
											//CodeStatement[] _get = null, _set = null;
											//string stmp = "stmp";
											//Type typ = typeof(string);
											//ctr = new CodeTypeReference(typ);

											//if (IsByteType(property.Type))
											//{
											//	if (IsArray(property.Type))
											//	{
											//		// if an array change the attribute to en element
											//		typeAttribute = null;// new CodeTypeReference(typeof(string));
											//		cad = new CodeAttributeDeclaration(new CodeTypeReference(typeof(System.Xml.Serialization.XmlElementAttribute)));
											//		// 
											//		cad = cad = new CodeAttributeDeclaration(new CodeTypeReference(typeof(System.Xml.Serialization.XmlElementAttribute)));

											//	}
											//	typeElement = new CodeTypeReference(typeof(string));

											//	_get = new CodeStatement[]
											//	{
											...
											//	_set = new CodeStatement[]
											//	{
											//		// set the attribute value directly
											//		new CodeAssignStatement(
											//			new CodeFieldReferenceExpression(
											//				new CodeThisReferenceExpression(),
											//				property.Name),
											...
											//	};
											//}
											//else if (IsByteType(property.Type))
											//{
											//	typeAttribute = null;// new CodeTypeReference(typeof(string));
											//	typeElement = property.Type;// new CodeTypeReference(typeof(string[]));
											...
											//}
											//else
											if (IsArray(property.Type) || property.UserData.Contains(isList))
											{
												//if (IsByteType(property.Type))
												//{
												//	property.Type = propertyField.Type = new CodeTypeReference(typeof(string));
												//	property.UserData.Remove(isList);
												//}

												//change attribute to element
												RemoveAttribute(property.CustomAttributes, XML_ATTRIBUTE_ATTRIBUTE);
												AddAttribute(property.CustomAttributes,
												...
												if ((property.UserData.Contains(isByteSequence)))
												{
													// add an element allowing to manipulate the array through a string
													/*
													 * create an XML element AND an XML attribute allowing to manipulate the array through a string
													 */
													CodeMemberProperty newElement = new CodeMemberProperty()
													{
														Attributes = property.Attributes,
														Name = property.Name + NexoXSDStrings.AsElement,
													...
													newElement = new CodeMemberProperty()
													{
														Attributes = property.Attributes,
														Name = property.Name + NexoXSDStrings.AsAttribute,
														CustomAttributes = new CodeAttributeDeclarationCollection()
														{
														new CodeAttributeDeclaration(
													...
													newElement.GetStatements.Clear();
													newElement.GetStatements.Add(
														new CodeState
														new CodeMethodReturnStatement(
															new CodeMethodInvokeExpression(
																new CodeMethodReferenceExpression(
																	new CodeTypeReferenceExpression(typeof(BitConverter)),
																	toString,
															...
															new CodePropertyReferenceExpression(
																new CodeThisReferenceExpression(),
																property.Name)));
													newElement.SetStatements.Clear();
													newElement.SetStatements.Add(new CodeAssignStatement(
														new CodeFieldReferenceExpression(
															new CodeThisReferenceExpression(),
															property.Name),
														new CodePropertySetValueReferenceExpression()));
													MakeIgnored(newElement, false);
													addedMembers.Add(newElement);

													/*
													 * make the element will not be serialized (the attribute will if necessary)
													 */
													newElement = new CodeMemberProperty()
													{
														Attributes = property.Attributes,
														Name = property.Name + NexoXSDStrings.AsElement + NexoXSDStrings.Specified,
														Type = new CodeTypeReference(typeof(bool)),
													};
													cs = new CodeMethodReturnStatement(
															new CodePrimitiveExpression(false));
													newElement.GetStatements.Add(cs);
													AddXmlIgnoreAttribute(newElement.CustomAttributes);
													addedMembers.Add(newElement);
												...
												//						new CodeThisReferenceExpression(),
												//						property.Name),
												//					new CodeDefaultValueExpression(typeAttribute))),
												//		})
												//};

												//_set = new CodeStatement[]
												//{
												//	// declare a new variable of the same base type (not an array) and set its value to default
												//	new CodeVariableDeclarationStatement(
												//			ctr,
												//			stmp),
												//	new CodeAssignStatement(
												//		new CodeVariableReferenceExpression(stmp),
												//		new CodeDefaultValueExpression(ctr)),
												//	new CodeTryCatchFinallyStatement(
												//		new CodeStatement[]
												//		{
												//			// declare the counter
												...
												//				isCounterI,
												//				new CodePrimitiveExpression(0)),
												//			// for loop to gather all values into 1 single string
												...
												//					new CodeConditionStatement(
												//						new CodeBinaryOperatorExpression(
												//							new CodePrimitiveExpression(isCounterI),
												...
												//									//"ToString"))),
												//						// else s = s + SEPARATOR +value[i].ToString()
												//						new CodeAssignStatement(
												//							new CodeVariableReferenceExpression(stmp),
												//							new CodeBinaryOperatorExpression(
												//								new CodeVariableReferenceExpression(stmp),
												//								CodeBinaryOperatorType.Add,
												//								new CodeBinaryOperatorExpression(
												//									new CodePrimitiveExpression(ATTRIBUTE_VALUE_SEPARATORS[0]),
												//									CodeBinaryOperatorType.Add,
												//									//new CodeMethodReferenceExpression(
												//										new CodeArrayIndexerExpression(
												//											new CodePropertySetValueReferenceExpression(),
											...
											{
												/*
												 * create an XML element alongside the XML attribute
												 * that element will not persist and is only here to avoid missing attributes specified as elements
												 */
												CodeMemberProperty newElement = new CodeMemberProperty()
												{
													Attributes = property.Attributes,
												...
												newElement.GetStatements.Clear();
												newElement.GetStatements.Add(new CodeMethodReturnStatement(
													new CodePropertyReferenceExpression(
														new CodeThisReferenceExpression(),
														property.Name)));
												newElement.SetStatements.Clear();
									...
									#region XmlText management
									try
									{
										/*
										 * adapt how XmlTextATtribute is declared inside the class
										 */
										cad = new CodeAttributeDeclaration(new CodeTypeReference(typeof(System.Xml.Serialization.XmlTextAttribute)));
										if (settings.AdaptXmlText
											&& null != (cad = LookForAttribute(property, cad))
											&& !IsArray(property.Type)
											&& !property.UserData.Contains(isList)
											&& IsIntegralType(property.Type)
											&& !IsStringType(property.Type))
										{
											/////*
											//// * if not a string and an integral type let's change it and create an alternate property as a double
											//// */
											//pname = property.Name + NexoXSDStrings.AsNumeric;

											/////*
											//// * create the intermediate property
											//// */
											//CodeMemberProperty newProperty = CreatePropertyMember(pname, property.Type, MemberAttributes.Public | MemberAttributes.Final);
											//AddXmlIgnoreAttribute(newProperty.CustomAttributes);
											//cs = new CodeMethodReturnStatement(
											//	new CodePropertyReferenceExpression(
											//		new CodeThisReferenceExpression(),
											//		propertyField.Name));
											//newProperty.GetStatements.Add(cs);
											//cs = new CodeAssignStatement(
											//	new CodeFieldReferenceExpression(
											//		new CodeThisReferenceExpression(),
											//		propertyField.Name),
											//	new CodePropertySetValueReferenceExpression());
											//newProperty.SetStatements.Add(cs);
											//addedMembers.Add(newProperty);

											/////*
											//// * update the real property to be a string and change get and set
											//// */
											...
											//cad.Arguments.Add(
											//	new CodeAttributeArgument(
											//		new CodeTypeReferenceExpression(property.Type)
											//		)
											//	);
									...
									if (null != propertySpecifiedProperty)
									{
										...
										cs = new CodeMethodReturnStatement(
											...
											new CodeBinaryOperatorExpression(
												...
												IsArray(property.Type) || property.UserData.Contains(isList)
												? (CodeExpression)new CodeBinaryOperatorExpression(
													...
													new CodePropertyReferenceExpression(
														new CodeFieldReferenceExpression(
															new CodeThisReferenceExpression(),
															propertyField.Name),
														settings.UseListInsteadOfArray ? isCount : isLength),
													CodeBinaryOperatorType.GreaterThan,
													new CodePrimitiveExpression(0))

												: IsEnumButNotAsString()
												? (CodeExpression)new CodePrimitiveExpression(false)

												: null != ctdOriginallyDeclaredInNamespace
												// hasbeenset
									...
									if (propertyToProcessInHasBeenSet)
									{
										...
										else if (IsEnumButNotAsString())
										{
											propertyField.InitExpression = new CodeObjectCreateExpression(propertyField.Type, new CodeExpression[] { });
											primitivesToProcessInHadBeenSet.Add(property);
										}
										// the field is a List of something...
										else if (property.UserData.Contains(isList))
										{
											propertyField.InitExpression = new CodeObjectCreateExpression(propertyField.Type, new CodeExpression[] { });
											arraysToProcessInHadBeenSet.Add(property);
										}
										...
										// the property is not a System.<Type>, isn't an array, isn't a specific type, we provide an initializer
										else if (null != ctdOriginallyDeclaredInNamespace)
										{
										...
										if (property.HasSet)
										{
											...
											cs = new CodeConditionStatement(
												...
												new CodeStatement[]
												{
													...
													new CodeAssignStatement(
														new CodeFieldReferenceExpression(
															new CodeThisReferenceExpression(),
															//propertySpecifiedPropertyField.Name),
															propertySpecifiedProperty.Name),
														new CodePrimitiveExpression(false))
												},

												// else
												IsEnumButNotAsString()
												? SetEnumValueStatements(property,
													new CodeAssignStatement(
														new CodeFieldReferenceExpression(
															new CodeThisReferenceExpression(),
															//propertySpecifiedPropertyField.Name),
												...
												// type defined inside the XSD but no special processing
												: null != ctdOriginallyDeclaredInNamespace
												? new CodeStatement[]
												{
													// property=value
												...
												// type not defined inside the XSD
												: new CodeStatement[]
												{
													// property=value
													new CodeAssignStatement(
									...
									{
										#region jagged array management
										//// is jagged array
										//bool isJaggedArray = false;
										//int nestingLevel = 0;
										//foreach (CodeAttributeDeclaration k in property.CustomAttributes)
										//{
										//	if (k.AttributeType.BaseType.Compare("System.Xml.Serialization.XmlArrayItemAttribute")  == CodeTypeReference("System.Xml.Serialization.XmlArrayItemAttribute") 0!=k.Arguments.Count && k.Arguments.Contains
										//} 
										//bool isJaggedArray = 0 != property.CustomAttributes.Count && 
										#endregion
												...
												new CodeMethodReturnStatement(
													new CodePropertyReferenceExpression(
														new CodeFieldReferenceExpression(
															new CodeThisReferenceExpression(),
															propertyField.Name),
														IsArray(property.Type) ? isLength : isCount))
											});
...
#if true
										CodeTypeReference a1 = new CodeDefaultValueExpression(property.Type).Type;
										CodeTypeReference a2 = new CodeTypeReference(property.Type.BaseType);
										//CodeTypeReference a3 = new CodeTypeReference(property.UserData[isArrayElementType].ToString());
										...
										cmm.Parameters.Add(cpde);
										cpde = new CodeParameterDeclarationExpression(
											IsArray(property.Type) ? new CodeTypeReference(property.Type.BaseType) : new CodeTypeReference(property.UserData[isArrayElementType].ToString()),
											isValue);
										cmm.Parameters.Add(cpde);
							...
							{
								if (
									mbr is CodeMemberProperty
									&& !mbr.UserData.Contains(NexoXSDStrings.UserDataKey)
						...
						else if (visible && codeType.IsEnum)
						{
							...
							// make it serializable as a string
							SerializeAsAString(codeType.CustomAttributes);
							...
							if (settings.AddDefaultAttributesToEnum)
							{
								...
								// _none
								cm.InitExpression = new CodePrimitiveExpression(0 /*settings.UseEnumBeginEnd ? -2 : -1*/);
								codeType.Members.Insert(i++, cm);
								if (settings.UseEnumBeginEnd)
								{
									...
									cm.InitExpression = new CodePrimitiveExpression(0);
									codeType.Members.Insert(i, new CodeMemberField("System.Int32", NexoXSDStrings.EnumBegin));
									// _end
				...
				if (settings.AddDispID && settings.DeclareClassInterface)
					newCodeNamespace.Imports.Add(new CodeNamespaceImport("System.Text.RegularExpressions"));
				#endregion
			}
			catch (Exception ex)
			{
		...
		private static CodeTypeReference ConvertType(XSDSettings settings, XSDParams parameters, CodeTypeReference type)
		{
			...
			else if (settings.DateTimeToString && settings.ConvertTypes && type.BaseType.Contains("DateTime"))
			{
			...
			// replace scalar type by string if replacement requested
				...
				//type.BaseType == typeof(System.Double?).ToString() ||
				//type.BaseType == typeof(System.Decimal?).ToString()
				type.BaseType.Contains("Int16") ||
				type.BaseType.Contains("Int32") ||
				type.BaseType.Contains("Int64") ||
				type.BaseType.Contains("UInt16") ||
				type.BaseType.Contains("UInt32") ||
				type.BaseType.Contains("UInt64") ||
				type.BaseType.Contains("Double") ||
				type.BaseType.Contains("Decimal") ||
				type.BaseType.Contains("Int16?") ||
				type.BaseType.Contains("Int32?") ||
				type.BaseType.Contains("Int64?") ||
				type.BaseType.Contains("UInt16?") ||
			...
			// replace scalar type by string if replacement requested
				...
				//type.BaseType == typeof(System.Boolean?).ToString()
				type.BaseType.Contains("Boolean") ||
				type.BaseType.Contains("Boolean?")
		...
		//			|| (null != parameters.ArrayTypesWithoutInitializer && !parameters.ArrayTypesWithoutInitializer.Contains(type.BaseType))));
		//}
		private static bool IsArray(CodeTypeReference type) => 0 != type.ArrayRank;
		private static bool IsObjectType(CodeTypeReference type) => IsType(type, "System.Object");
		private static bool IsStringType(CodeTypeReference type) => IsType(type, "System.String");
		private static bool IsByteType(CodeTypeReference type) => IsType(type, "System.Byte") || IsType(type, "System.SByte");
		private static bool IsType(CodeTypeReference type, string typ)
		{
			...
			if (null != t)
				...
				return t.FullName.Contains(typ);
			return false;
		}
		/// <summary>
		...
		private static void SerializeAsAString(CodeAttributeDeclarationCollection attrs)
		{
			AddAttribute(attrs, "Newtonsoft.Json.JsonConverter", new CodeAttributeArgument[] { new CodeAttributeArgument(new CodeTypeOfExpression($"Newtonsoft.Json.Converters.StringEnumConverter")) });
		}
		...
		/// <param name="attribute"></param>
		/// <param name="arguments"></param>
		private static void AddAttribute(CodeAttributeDeclarationCollection attrs, CodeAttributeDeclaration attribute, CodeAttributeArgument[] arguments = null)
		{
		...
		/// <summary>
		/// Remove an attribute by part of its name
		/// </summary>
		/// <param name="attrs">List of attributes</param>
		/// <param name="attribute">Attribute name to look for</param>
		private static void RemoveAttribute(CodeAttributeDeclarationCollection attrs, string attribute)
		{
			for (int i = 0; i < attrs.Count; i++)
			{
				if (attrs[i].Name.Contains(attribute))
				{
					attrs.RemoveAt(i);
				}
			}
		...
		private void MakeIgnored(CodeTypeMember prop, bool fullyIgnored)
		{
			prop.UserData.Add(NexoXSDStrings.UserDataKey, true);
			if (fullyIgnored)
				AddXmlIgnoreAttribute(prop.CustomAttributes);
			else
				AddAttribute(prop.CustomAttributes, JSON_IGNORE_ATTRIBUTE);
		}
		/// <summary>
		/// 
		/// </summary>
		...
		/// <param name="arg">The argument to look for, null if no specific argument to look for</param>
		/// <returns>The argument to look for, null if not found</returns>
		private CodeAttributeDeclaration LookForAttribute(CodeMemberProperty prop, CodeAttributeDeclaration decl, CodeAttributeArgument arg = null)
		{
			CodeAttributeDeclaration o = null;
			bool hasBeenFound = false;
			// loop on all property attributes
			foreach (CodeAttributeDeclaration xcad in prop.CustomAttributes)
			{
			...
			// return the attribute declaration, if any
			return o;
		}
		/// <summary>
