...
namespace Listener.Service
{
	public partial class ListenerService : ServiceBase
	{
		...
		const string EVENT_LOG_SOURCE = "listenerService.30";
		...
		const string REGISTRY_KEY_SETTINGS_FILE_NAME_DEFAULT = @"listener.service.30.settings.json";
		const string REGISTRY_KEY_LOG_FILE_NAME = @"Log";
		const string REGISTRY_KEY_LOG_FILE_NAME_DEFAULT = @"listener.service.30.log";
		#endregion
		...
		bool Start()
		{
			try
			{
				...
				if (listener.Start($"{fldSettings.FullName}\\{settingsFileName}").Result)
					return true;
			}
			catch (Exception ex)
			{
